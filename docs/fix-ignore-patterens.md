...
*   [ ] **Analyze `IgnorePatternsProps` Mismatches:**
    *   Review the `IgnorePatternsProps` interface definition in `src/components/IgnorePatterns.tsx`.
    *   Compare the defined props with those being passed in `src/components/Sidebar.tsx` (line 646). Fix the props being passed (e.g., pass `globalPatternsState`, `localPatternsState`, `onExcludedSystemPatternsChange` instead of `globalIgnorePatterns`, `localIgnorePatterns`, etc.).
    *   Update the component calls within `src/components/__tests__/IgnorePatterns.test.tsx` to match the correct `IgnorePatternsProps` interface, providing the required state objects and callbacks.
*   [ ] **Fix `excludedSystemPatterns` Access in `App.tsx`:**
    *   Go to `src/App.tsx` line 278.
    *   Inspect the return type of `parseIgnorePatternsContent`.
    *   Correct the code to access the intended property (likely `excludedPatterns` based on the function definition in `src/utils/patternUtils.ts`).
*   [ ] **Address `SearchBar` Prop Type Issue:**
    *   Investigate the errors in `src/components/Sidebar.tsx` (lines 401, 584) where `SearchBar` is rendered.
    *   Verify the props interface used by the `SearchBar` component definition. Ensure it's using its own `SearchBarProps` (defined in `src/types/FileTypes.ts`) and not accidentally typed as `ExtendedSidebarProps`.
    *   Correct the type definition for the `SearchBar` component if it's wrong.
*   [ ] **Resolve `resetIgnorePatterns` Signature Mismatch:**
    *   Examine the `resetIgnorePatterns` prop definition in `IgnorePatternsProps` and compare it with the function signature being passed in `src/components/Sidebar.tsx` (line 646, or potentially line 449 if it was moved).
    *   Ensure the types match regarding `async`/`Promise` and parameter types (`string` vs `string | undefined`). Adjust the prop definition or the passed function/callback.
*   [ ] **Fix Missing `content` Property in `FileList.tsx`:**
    *   Go to `src/components/FileList.tsx` line 32.
    *   Determine why `FileCard` requires `content` but `FileList` receives files of type `Omit<FileData, 'content'>`.
    *   Decide on a fix:
        *   Option A: Modify `App.tsx` or the data fetching logic to include `content` when passing `allFiles` (if feasible and performant).
        *   Option B: Modify `FileList` to fetch content for selected files before rendering `FileCard` (might require async operations).
        *   Option C: Modify `FileCard` to not strictly require `content` or fetch it internally if needed (e.g., for the copy button).
...
*   [ ] **Run Type Checking:** After applying fixes, run `tsc --noEmit` again to ensure all TypeScript errors are resolved.
*   [ ] **Run Linter:** Run `npm run lint` to check if warnings are resolved or reduced.
